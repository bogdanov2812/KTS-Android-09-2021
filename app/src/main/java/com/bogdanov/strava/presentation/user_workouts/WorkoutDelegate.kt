...
class WorkoutDelegate: AbsListItemAdapterDelegate<Workout, Workout, WorkoutDelegate.ViewHolder>(){
    ...
    inner class ViewHolder(
        ...
        fun bind(item: Workout) = with(viewBinding) {
            name.text = item.name
            date.text = reformatDate(item.startDate)
            timeTrain.text = reformatTime(item.startDate)
            distance.text = item.distance.toString()
            temp.text = "%.2f".format(item.averageSpeed)
            time.text = timeToMinute(item.movingTime).toString()
        }
